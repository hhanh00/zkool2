...
pub async fn z2z<D: Device, G: Deref<Target = D>, R: RngCore + CryptoRng>(
    ...
    let pool = SqlitePool::connect("nano.db").await?;
    ...
    println!("Reading original pczt");
    let file = File::open("z2z.bin")?;
    let reader = BufReader::new(file);
    let package: PcztPackage = bincode::decode_from_reader(reader, bincode::config::legacy())?;
    ...
    println!("PCZT structure: {ctin}/{ctout} : {stin}/{stout}");
    ...
    for sp in pczt.sapling().spends().iter() {
        ...
        let (value, diversifier): (u64, Vec<u8>)
         = sqlx::query_as(
    ...
    if stage == 2 {
        ...
        for sp in pczt.sapling().spends().iter() {
            println!("Extracting spend randomness");
            let res = ledger.execute(&xtract_sp).await?;
            println!("Got spend randomness");
            assert_eq!(res.retcode, 0x9000);
            let data = &res.data;
            nsk = tiu!(data[32..64]);
        ...
        for out in pczt.sapling().outputs().iter() {
            println!("Extracting output randomness");
            let res = ledger.execute(&xtract_out).await?;
            println!("Got output randomness");
            assert_eq!(res.retcode, 0x9000);
            let data = &res.data;
            let rcv = &data[0..32];
        ...
        println!("Creating new PCZT");
        let sbundle =
            pczt::sapling::Bundle::from_parts(sins, souts, value_sum, anchor.unwrap(), None);
        let pczt =
        ...
        println!("Adding proofs to PCZT");
        let pczt = Prover::new(pczt)
            .create_sapling_proofs(prover, prover)
            .unwrap()
            .finish();

        let pczt_file = File::create("z2z-proven.bin")?;
        let mut writer = BufWriter::new(pczt_file);
        writer.write_all(&pczt.serialize())?;
        ...
        let tot = buffers.iter().map(|d| d.len()).sum::<usize>();

        println!("Data len = {tot}");
        println!("Compute hashes");
        ...
        println!("Submitting new tx to Ledger");
    ...
    if stage == 3 {
        ...
        for _ in pczt.transparent().inputs() {
            println!("Getting transparent signature");
        ...
        for _ in pczt.sapling().spends() {
            println!("Getting sapling signature");
        ...
        let pczt_file = File::open("z2z-proven.bin")?;
        let mut reader = BufReader::new(pczt_file);
        let mut bytes = vec![];
        ...
        let (svk, ovk) = prover.verifying_keys();
        ...
        let coin = Coin::new(ServerType::Lwd, "https://zec.rocks", false, "nano.db", None).await?;
        let mut client = coin.client().await?;
        let rep = client.post_transaction(expiry_height, &tx_bytes).await?;
        println!("{rep}");
    }
    Ok(())
}
