...
use rand_core::OsRng;
use reddsa::frost::redpallas::frost::{self, Identifier};
...
impl DKGState {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        self.create_broadcast_account(m.to_string().as_str()).await?;

        let id = self.package.id as u16;
        let (sk1, pk1) = frost::keys::dkg::part1::<RedPallas, _>(
            id.try_into().unwrap(),
            self.package.n as u16,
            self.package.t as u16, OsRng)?;
        let pk1 = DKGPackage {
            from_id: id,
            payload: pk1.serialize()?,
        };

        let sk1 = bincode::serde::encode_to_vec(&sk1, config::standard())?;
        sqlx::query(
