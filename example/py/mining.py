...
transport = gql.transport.aiohttp.AIOHTTPTransport(
    url="http://localhost:8000/graphql", timeout=60
)
...
def run(miner_seed: str, seed: str, to_address: str):
    ...
    all_notes = client.execute(
        gql.gql("""
        query NotesByAccount($id: Int!) {
            notesByAccount(idAccount: $id) { id height value }
        }
    """),
        variable_values={"id": miner},
    )["notesByAccount"]

    notes = [n for n in all_notes if n["height"] < height - MATURITY_THRESHOLD][
        :MAX_NOTES
    ]
    if not notes:
...
run(os.environ["MINER_SEED"], os.environ["SEED"], os.environ["DESTINATION_ADDRESS"])
